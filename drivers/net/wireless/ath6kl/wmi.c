...
int ath6kl_wmi_add_pkt_filter_pattern_cmd(struct wmi *wmi, u8 if_idx,
					  u8 filter_id, u8 filter_act, u8 filter_size,
					  u8 filter_offset, u8 *filter, u8 *mask)
{
	...
	ret = ath6kl_wmi_cmd_send(wmi, if_idx, skb, WMI_ADD_PKT_FILTER_PATTERN_CMDID,
				  NO_SYNC_WMIFLAG);

	return ret;
}

int ath6kl_wmi_del_pkt_filter_pattern_cmd(struct wmi *wmi, u8 if_idx, u8 filter_id)
{
	struct sk_buff *skb;
	...
	ret = ath6kl_wmi_cmd_send(wmi, if_idx, skb, WMI_DEL_PKT_FILTER_PATTERN_CMDID,
				  NO_SYNC_WMIFLAG);
	return ret;
}
