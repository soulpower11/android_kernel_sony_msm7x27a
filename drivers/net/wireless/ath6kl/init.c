...
static int ath6kl_set_reg_dmn(struct ath6kl *ar)
{
	...
	ret = ath6kl_bmi_read(ar,
			ath6kl_get_hi_item_addr(ar, HI_ITEM(hi_board_data)),
			(u8 *)&brd_dat_addr, 4);
	if (ret)
		return ret;
...
/*++ FelexChing - 20121010 Add for wifi calibrate ++*/
...
int get_wifi_cali(char* buff_input)
{
	...
	for(tmp = 0; tmp < 16; tmp++){
	    ...
	    if(strncmp(ret.output, err_msg,sizeof(err_msg)) == 0){
		 kfree(input);
	        kfree(output);
	        return -1;
	    }else
...
static int ath6kl_fetch_board_file(struct ath6kl *ar)
{
	...
	if (ret == 0) {
/*++ FelexChing - 20121010 Add for wifi calibrate ++*/
           ret = get_wifi_cali(ar->fw_board);
           if(ret == -1)
	        ath6kl_err("Using Default Bdata.bin\n");
/*++ FelexChing - 20121010 Add for wifi calibrate ++*/
