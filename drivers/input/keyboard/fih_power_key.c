...
int	get_interval( unsigned long old, unsigned long new )
{
	...
	unsigned int old_msec = jiffies_to_msecs( old ), new_msec = jiffies_to_msecs( new );

	return	old_msec > new_msec ? jiffies_to_msecs( LONG_MAX ) - old_msec + new_msec : new_msec - old_msec;
	...
	case KEY_END:
		if( key == KEY_POWER || key == KEY_END )
		{
			...
			if( key_code != HS_REL_K )
			{
				...
				power_key_data->tick_time		= store_jiffies;

				up( &tick_time_protection );

				power_key_data->set_interval	= 1;

				printk( "PKEY : Key event(%ums)\n", jiffies_to_msecs( store_jiffies ) );
...
static ssize_t power_key_control( void *node_data, struct device_attribute *attr, const char *buf, size_t size )
{
	...
	count	= get_para_from_buffer( buf, "%d%d", &ID, &interval );

	if( !count )
		printk( "PKEY : get_para_from_buffer() return %d\n", count );
...
void	check_power_key_skip_count( void )
{
	...
	else
	{
		...
		power_key_data->tick_time = store_jiffies = store_jiffies - msecs_to_jiffies( power_key_data->interval );
